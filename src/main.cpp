...
int main() {
    ...
    std::vector<std::string> availableLanguages
      = getAvailableLanguages(addpath + load_json<std::string>(config, ("languagepath")));
    ...
    auto render = [&]() {
        ...
        if(!sampler.has_value()) {
            ImGui::SameLine();
            if(ImGui::Button("Speichern", ImVec2(load_json<Size>(config, "button")))) {
            ...
            if(ImGui::Button("Analysiere Daten", ImVec2(load_json<Size>(config, "button")))) {
                // fileBrowser.Open();
            }
            /*
            //fileBrowser.Display();
            ...
            */
            ...
            // ##############################
            if(ImGui::Button("Erstelle Lerndatensatz", ImVec2(load_json<Size>(config, "button")))) {
