...
static void show_standart_input(
  ...
  std::string&          comment_string) {
    ...
    ImGui::BeginChild("trainingleft", ImVec2(windowSize.x * 0.5f, windowSize.y * 0.8f));
    ImGui::TextUnformatted(load_json<std::string>(language, "training", "base_data").c_str());
    ImGui::InputText(
    ...
    ImGui::SameLine();
    ImGui::Checkbox(load_json<std::string>(language, "training", "problem").c_str(), &problem);
    if(problem == maintenance) {
        maintenance = !problem;
    }

    ImGui::TextUnformatted(
      load_json<std::string>(language, "training", "electrical_consumer").c_str());
    ImGui::Checkbox(
    ...
    ImGui::TextUnformatted(load_json<std::string>(language, "training", "evaluation").c_str());
    ImGui::Checkbox(load_json<std::string>(language, "training", "normal_case").c_str(), &expected);
    if(expected == anomaly) {
        anomaly = !expected;
    }
    ImGui::SameLine();
    ImGui::Checkbox(load_json<std::string>(language, "training", "anomaly").c_str(), &anomaly);
    if(anomaly == expected) {
        expected = !anomaly;
    }
...
static void selected_vcds_data(
  ...
  bool&                 upload_success) {
    ImGui::TextUnformatted(
      load_json<std::string>(language, "explanation", "upload", "vcds").c_str());
         ...
         ImVec2(load_json<Size>(config, "button"))))
    {
        ...
        api_message = send_to_api(
          ...
          path1,
          inputvin,
          load_json<std::string>(language, "measuretype", "vcds"),
          metadata);
        upload_success = true;
...
inline void selected_battery_measurement(
  ...
  bool&                 upload_success) {
    ImGui::TextUnformatted(
      load_json<std::string>(language, "explanation", "upload", "battery").c_str());
    static ImGui::FileBrowser fileBrowser;
    static bool               first_job = true;
...
static void selected_compression_data(
  ...
  bool&                 upload_success) {
    ...
    ImGui::TextUnformatted(
      load_json<std::string>(language, "training", "compression", "max_compression").c_str());
    ...
    ImGui::Columns(2);
    ImGui::TextUnformatted(
      load_json<std::string>(language, "training", "compression", "ignition_suspended").c_str());
    ImGui::TextUnformatted(
      load_json<std::string>(language, "training", "compression", "ignition_active").c_str());
    ImGui::NextColumn();

    ImGui::InputText("##path1", path1, sizeof(path1));
    ImGui::SameLine();
    const std::string button1 = load_json<std::string>(language, "button", "file") + " 1";
    const std::string button2 = load_json<std::string>(language, "button", "file") + " 2";
...
inline void popup_create_training_data_select(
  nlohmann::json const& config,
  nlohmann::json const& language,
  bool&                 upload_success) {
    static int  selectedOption = 1;   // Standardauswahl
    char const* options[]      = {"Kompressionsmessung", "Batteriemessung", "VCDS-Datei"};
    ...
    ImVec2 windowSize = ImGui::GetWindowSize();
    ImGui::BeginChild("trainingright", ImVec2(windowSize.x * 0.5f, windowSize.y * 0.8f));
    switch(selectedOption) {
    case 0:
        selected_compression_data(
          config,
          language,
          metadata,
          inputvin,
    ...
    case 2:
        selected_vcds_data(
          ...
          upload_success);
        break;
    default: ImGui::Text("Fehler, Switchcase mit unerwarteter Auswahl");
    }
    if(ImGui::Button(
