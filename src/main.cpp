...
static void set_button_style_to(nlohmann::json const &config, std::string const &name)
{
    // set rgb and transparency for "name" on normal/hover/active
    ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(load_json<Color>(config, "button", name, "normal")));
    ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(load_json<Color>(config, "button", name, "hover")));
    ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImVec4(load_json<Color>(config, "button", name, "active")));
}

static void load_settings(nlohmann::json const &config)
{
    // set rgb and transparency on background of main/popup/normal/color
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(load_json<Color>(config, "text", "color", "normal")));
...
int main()
{
    ...
    nlohmann::json language = load_json_file(load_json<std::string>(config, "languagepath") +
    ...
    auto render = [&]()
    {
        load_settings(config); // set rgb and transparency on background 
        ImGui::SetupImGuiStyle(false, 0.99f); // access the Style structure (colors, sizes) 
        ...
        {
            if (ImGui::BeginMenu(load_json<std::string>(language, "menubar", "menu", "language_option").c_str()))
            {
        ...
        if (ImGui::BeginMenu(load_json<std::string>(language, "menubar", "help", "label")
                    .c_str()))
        {
        ...
        mainMenuBarSize = ImGui::GetItemRectSize(); // get size of last item
        ...
        if (flagPaused)
        {
            ...
            ImGui::Button(
                   load_json<std::string>(language, "button", "analyse_data").c_str(), toolBtnSize);
            ...
            if (ImGui::Button(load_json<std::string>(language, "button",
                             "create_training_data").c_str(), toolBtnSize))
            {
        ...
        else
        {
            ...
            ImGui::SameLine();
            ImGui::Button(
                   load_json<std::string>(language, "button", "create_training_data").c_str(), 
                   toolBtnSize);
            ImGui::PopStyleColor();
        }
        ImGui::EndChild();
        ...
        // ##############################
        // std::string settingstitle = load_json<std::string>(language, "generate_training_data", "title");
        ...
        {
            static int e = 0;
            ImGui::RadioButton("User current Waveform", &e, 0);
            ImGui::RadioButton("Waveform from File", &e, 1);
            ...
            static int a = 0;
            ImGui::RadioButton("Maintenance", &a, 0);
            ImGui::SameLine();
            ImGui::RadioButton("Fault", &a, 1);
    ...
    // used glfwWindowHint(GLFW_MAXIMIZED, GLFW_TRUE); in ImGuiInstance.hpp
