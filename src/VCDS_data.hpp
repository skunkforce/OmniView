...
static void select_vcds_data(nlohmann::json const &config, char* PATH2) {
  ...
  if (first_job_VCDS) {
    fileBrowser_VCDS.SetPwd(load_json<std::filesystem::path>(config, "scanfolder"));
...
inline void popup_create_training_data_select(nlohmann::json const &config,
  ...
  // Concept: Sending data with : VIN, MILEAGE, MESSAGE (First Version) in PATH1 --> Needs to be selected
  // Sending VCDS data to a different API via PATH2 with the same sending button --> optional selectable
  // Sending Symptoms to a different API with the same sending button --> optional selectable 

  static int selectedOption = 1; // number of selectable components/ cases (NOT Hub Cases)
  const char *options[] = {"Batteriemessung"}; // possible options 
  ImGui::Combo("Messung", &selectedOption, options, IM_ARRAYSIZE(options)); // Drop Down Menu 
 ...
 // Declare variables 
  ...
  // setting the first menu part --> Choosing the PATH1 for the data as well as all needed Metadata NOT optional data 
   ...
   static bool test = false; 
    ...
    if(ImGui::Button("SENDEN", ImVec2(load_json<Size>(config, "button")))){
        switch(selectedOption){
            case 0: 
                send_battery_measurement(config, 
                                metadata,
                                inputvin,
                                mileage,
